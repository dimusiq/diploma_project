...
function normalizeIntervalHours(val: unknown): number[] {
  if (Array.isArray(val)) {
    const nums = val.filter((x): x is number => typeof x === 'number' && x > 0 && Number.isInteger(x));
    return nums;
  }
    ...
    return arr
      .filter(
        (x): x is Record<string, unknown> =>
          typeof x === 'object' && x != null && typeof (x as Record<string, unknown>).id === 'string' && typeof (x as Record<string, unknown>).name === 'string'
      )
      .map((x) => {
        const color = typeof x.colorTag === 'string' && x.colorTag && !GRAPH_STATUS_COLORS.includes(x.colorTag as (typeof GRAPH_STATUS_COLORS)[number])
          ? x.colorTag
          : DEFAULT_CHAIN_COLOR;
        const remind = typeof x.remindBeforeHours === 'number' && x.remindBeforeHours >= 0
          ? x.remindBeforeHours
        ...
        return {
          ...
          equipmentIds: Array.isArray(x.equipmentIds) ? (x.equipmentIds as string[]) : [],
        };
      });
  } catch {
    return [];
...
export function saveMaintenanceChain(chain: Omit<MaintenanceChain, 'id'> & { id?: string }): MaintenanceChain {
  const list = getMaintenanceChains();
  const id = chain.id ?? generateId();
  const allowedColors = CHAIN_COLOR_OPTIONS.map((o) => o.value);
  const colorTag = typeof chain.colorTag === 'string' && allowedColors.includes(chain.colorTag)
    ? chain.colorTag
    : DEFAULT_CHAIN_COLOR;
  const remindBeforeHours = typeof chain.remindBeforeHours === 'number' && chain.remindBeforeHours >= 0
    ? chain.remindBeforeHours
    : DEFAULT_REMIND_BEFORE_HOURS;
  const newChain: MaintenanceChain = {
    id,
    name: String(chain.name).trim(),
    intervalHours: Array.isArray(chain.intervalHours) ? chain.intervalHours.filter((x) => x > 0 && Number.isInteger(x)) : [],
    colorTag,
    remindBeforeHours,
    equipmentIds: Array.isArray(chain.equipmentIds) ? chain.equipmentIds : [],
  };
  const index = list.findIndex((c) => c.id === id);
  const next = index >= 0 ? list.map((c, i) => (i === index ? newChain : c)) : [...list, newChain];
...
export function getRemindBeforeHoursForEquipment(equipmentId: string, defaultRemind: number): number {
  const chains = getMaintenanceChains().filter((c) => c.equipmentIds.includes(equipmentId));
...
export function getEquipmentIdsInOtherChains(excludeChainId: string | null): Set<string> {
...
export function getChainNameForEquipment(equipmentId: string, excludeChainId: string | null): string | null {
  const chain = getMaintenanceChains()
    .filter((c) => c.id !== excludeChainId && c.equipmentIds.includes(equipmentId))
