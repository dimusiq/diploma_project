...
import { EQUIPMENT_TYPE_LABELS } from '@/api/equipment';
import type { EquipmentPublic } from '@/api/equipment';

export type ScheduleStatus = 'overdue' | 'due_soon' | 'ok';
...
function rowToCells(r: MaintenanceScheduleRowExport): (string | number)[] {
  const remaining =
    r.engineHours != null && r.nextServiceAtHours != null && r.engineHours < r.nextServiceAtHours
      ? r.nextServiceAtHours - r.engineHours
      : null;
  return [
    `${r.equipment.brand_name ?? ''} ${r.equipment.model ?? ''}`.trim(),
    EQUIPMENT_TYPE_LABELS[r.equipment.equipment_type] ?? r.equipment.equipment_type,
    r.equipment.serial_number ?? '—',
    r.lastMaintenanceAtHours ?? '—',
    r.engineHours ?? '—',
    r.nextServiceAtHours ?? '—',
    remaining != null ? remaining : (r.status === 'overdue' ? '0 (просрочено)' : '—'),
    r.primaryChainName || '—',
...
export function downloadMaintenanceScheduleCsv(filteredRows: MaintenanceScheduleRowExport[]): void {
...
export async function downloadMaintenanceScheduleXlsx(filteredRows: MaintenanceScheduleRowExport[]): Promise<void> {
  const ExcelJS = (await import('exceljs')).default;
  const wb = new ExcelJS.Workbook();
  const ws = wb.addWorksheet('График ТО', { views: [{ state: 'frozen', ySplit: 1 }] });
