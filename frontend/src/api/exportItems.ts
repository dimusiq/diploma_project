 ...
 */
export async function downloadItemsExport(params: ExportItemsParams): Promise<void> {
  ...
  if (!res.ok) {
    const msg = res.status === 400 ? 'Неверный формат (ожидается csv или xlsx)' : `Ошибка: ${res.status}`;
    throw new Error(msg);
  }
  const blob = await res.blob();
  const disposition = res.headers.get('Content-Disposition');
  const match = disposition?.match(/filename=(.+)/);
  const filename = match ? match[1].replace(/^["']|["']$/g, '') : `items_export.${params.format === 'xlsx' ? 'xlsx' : 'csv'}`;
