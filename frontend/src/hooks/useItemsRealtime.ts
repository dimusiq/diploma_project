/**
 * SSE GET /api/v1/items/stream — обновление кэша товаров при изменениях на сервере.
 * Альтернатива: WebSocket ws://.../api/v1/items/ws?token=... (те же JSON-сообщения).
 */
...
async function consumeItemsStream(
  signal: AbortSignal,
  ...
  const res = await fetch(getApiUrl("/api/v1/items/stream"), {
...
export function useItemsRealtime() {
  ...
  useEffect(() => {
    ...
    const loop = async () => {
      ...
      try {
        await consumeItemsStream(sig, (raw) => {
          try {
