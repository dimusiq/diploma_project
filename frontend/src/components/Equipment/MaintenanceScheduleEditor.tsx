...
function chainTagBadgeClass(tag: string): string {
  return (
    CHAIN_TAG_BADGE_CLASS[tag] ?? CHAIN_TAG_BADGE_CLASS.blue
  );
}
...
const STORAGE_KEY_IMPORT_DONE =
  'maintenance_schedule_import_done';
...
export function MaintenanceScheduleEditor() {
  ...
  const [deleteChainId, setDeleteChainId] = useState<
    string | null
  >(null);
  ...
  const [chainColorTag, setChainColorTag] =
    useState<string>('blue');
  const [chainIntervals, setChainIntervals] = useState<
    number[]
  >([]);
  const [selectedEquipmentIds, setSelectedEquipmentIds] =
    useState<Set<string>>(new Set());
  const [addIntervalValue, setAddIntervalValue] =
    useState<number>(500);
  const [
    chainRemindBeforeHours,
    setChainRemindBeforeHours,
  ] = useState<number>(DEFAULT_REMIND_BEFORE_HOURS);
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem(STORAGE_KEY_IMPORT_DONE))
      return;
  ...
  const setIntervals = (
    ...
    const value =
      typeof next === 'function' ? next(intervals) : next;
  ...
  const isFormDirty = useMemo(() => {
    ...
    const remindMatch =
      chainRemindBeforeHours === s.remindBeforeHours;
    const intervalsMatch =
      chainIntervals.length === s.intervalHours.length &&
      chainIntervals.every(
        (v, i) => v === s.intervalHours[i],
      );
  ...
  const filteredEquipmentList = useMemo(() => {
    ...
    return equipmentList.filter(
      (e) =>
        ...
        String(
          EQUIPMENT_TYPE_LABELS[e.equipment_type] ??
            e.equipment_type,
        )
          .toLowerCase()
          .includes(q),
  ...
  const handleSaveChain = () => {
    ...
    if (name.length > 40) {
      setError(
        'Название цепочки ТО не должно превышать 40 символов.',
      );
      return;
    }
    ...
    if (duplicate) {
      setError(
        'Последовательность с таким названием уже существует.',
      );
      return;
    }
    if (chainIntervals.length === 0) {
      setError(
        'Добавьте хотя бы один интервал в последовательность',
      );
      return;
    }
    const inOther = Array.from(selectedEquipmentIds).filter(
      (id) => equipmentIdsInOtherChains.has(id),
    );
    if (inOther.length > 0) {
      const names = inOther
        .slice(0, 3)
  ...
  const handleDeleteChainConfirm = () => {
    ...
    deleteChainMutation.mutate(deleteChainId, {
      onSuccess: () => {
        if (editingChainId === deleteChainId)
          setEditingChainId(null);
  ...
  const handleUnsavedConfirm = () => {
    ...
    else if (unsavedConfirm.action === 'cancel')
      doCancelChainForm();
      ...
      <div className='flex flex-col gap-8'>
        ...
        <div id='sequence-to'>
          ...
          <p className='mb-4 text-sm text-muted-foreground'>
            Создайте последовательности из графика
            (интервала) и назначьте технику. Одна
            последовательность — один интервал и список
            техники.
          </p>
          ...
          {!canEdit && (
            <p className='mb-4 text-sm text-muted-foreground'>
              Только просмотр. Редактирование расписания
              недоступно.
            </p>
          )}
          {chains.length === 0 ? (
            <div className='mb-4 rounded-md border border-dashed border-border p-4 text-center'>
              ...
              <p className='mb-3 text-sm text-muted-foreground'>
                Создайте первую последовательность, выберите
                интервалы и назначьте технику.
              </p>
            </div>
          ) : (
            <div className='mb-4'>
          ...
          {editingChainId != null && (
            <div className='w-full rounded-md border border-border bg-muted/40 p-4'>
              ...
              <div className='flex flex-col gap-4'>
                ...
                <div className='flex flex-wrap items-center gap-2'>
                  <span className='w-[100px] text-sm font-medium'>
                    Цветовое обозначение последовательности
                    ТО:
                  </span>
                  <DropdownMenu>
                ...
                <div>
                  ...
                  <p className='mb-2 text-xs text-muted-foreground'>
                    ...
                    Первый интервал используется для расчёта
                    «следующее ТО» в графике.
                  </p>
                  {canEdit && (
                  ...
                  {chainIntervals.length === 0 ? (
                    <p className='text-sm text-muted-foreground'>
                      Нет интервалов. Выберите интервал выше
                      и нажмите «Добавить в цепочку».
                    </p>
                  ) : (
                ...
                <div>
                  ...
                  />
                  <p className='mb-2 text-xs text-muted-foreground'>
                    Техника может входить только в одну
                    последовательность ТО. Занятая в другой
                    последовательности техника недоступна
                    для выбора.
                  </p>
                  <div className='max-h-[400px] overflow-y-auto rounded-md border border-border bg-background p-2'>
                    {equipmentList.length === 0 ? (
                      <p className='text-sm text-muted-foreground'>
                        Нет техники. Добавьте технику в
                        разделе «Список техники».
                      </p>
                      ...
                      <div className='grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3'>
                        {filteredEquipmentList.map((eq) => {
                          ...
                          return (
                            <Checkbox
                              key={eq.id}
                              checked={selectedEquipmentIds.has(
                                eq.id,
                              )}
                              disabled={
                                inOtherChain || !canEdit
                              }
                              onCheckedChange={() =>
                                !inOtherChain &&
                                canEdit &&
        ...
        <div id='service-periods'>
          ...
          <p className='mb-4 text-sm text-muted-foreground'>
            Отметки в моточасах для ТО (500, 1000, 1500 и
            т.д.). Первый период используется в «График ТО»
            по умолчанию.
          </p>
          {canEdit && (
            <div className='mb-2 flex flex-wrap items-center gap-2'>
              <Input
                ...
                onKeyDown={(e) =>
                  e.key === 'Enter' &&
                  (e.preventDefault(), handleAddInterval())
                }
