...
type FormValues = Omit<EquipmentCreate, 'brand_id'> & { brand_id: string; instructions?: string | null };
...
export function EquipmentFormDialog({ open, onOpenChange, editItem, asDrawer = false, asPage = false }: EquipmentFormDialogProps) {
  ...
  const onSubmit: SubmitHandler<FormValues> = (data) => {
    ...
    if (attachmentsStr) {
      try {
        ...
        attachmentsParsed = attachmentsStr.split(/\n/).map((x) => x.trim()).filter(Boolean);
      }
    }
    const body: EquipmentCreate = {
      ...
      engine_hours: data.engine_hours != null ? Number(data.engine_hours) : null,
      current_status: data.current_status || 'active',
      zone: data.zone?.trim() || null,
      attachments: attachmentsParsed.length ? JSON.stringify(attachmentsParsed) : null,
  ...
  const formContent = (
    <>
      <Text mb={4}>
        Складская техника: тип, бренд (из справочника), VIN, модель, моточасы, место.
      </Text>
      {brands.length === 0 && (
        <Text fontSize="sm" color="fg.muted" mb={2}>
          Нет брендов. Добавьте их в разделе «Администрирование» → Бренды техники.
        </Text>
      )}
      <VStack gap={4}>
              <Field required invalid={!!errors.equipment_type} errorText={errors.equipment_type?.message} label="Тип техники">
                <select
                  id="equipment_type"
              ...
              <Field required invalid={!!errors.brand_id} errorText={errors.brand_id?.message} label="Бренд">
                <select
                  id="brand_id"
              ...
              <Field invalid={!!errors.vin} errorText={errors.vin?.message} label="VIN">
                <Input id="vin" {...register('vin')} placeholder="VIN" />
              </Field>
              <Field invalid={!!errors.serial_number} errorText={errors.serial_number?.message} label="Серийный номер">
                <Input id="serial_number" {...register('serial_number')} placeholder="Серийный номер" />
              </Field>
              <Field required invalid={!!errors.model} errorText={errors.model?.message} label="Модель">
                <Input
                  id="model"
              ...
              <Field invalid={!!errors.engine_hours} errorText={errors.engine_hours?.message} label="Моточасы">
                <Input
                  id="engine_hours"
                  type="number"
                  min={0}
              ...
              <Field label="Зона склада">
                {zones.length > 0 ? (
                  ...
                  >
                    ...
                    {editItem?.zone?.trim() && !zones.some((z) => z.name === editItem.zone) && (
                      <option value={editItem.zone}>{editItem.zone} (текущее)</option>
                    )}
                    {zones.map((z) => (
  ...
  if (asDrawer) {
    return (
      <DrawerRoot open={open} onOpenChange={(e) => onOpenChange(e.open)} placement="end" size="md">
        <DrawerBackdrop />
        <DrawerContent>
          <DrawerCloseTrigger />
          <form onSubmit={handleSubmit(onSubmit)}>
            <DrawerHeader>
              <DrawerTitle>{isEdit ? `${editItem?.brand_name} ${editItem?.model}` : 'Добавить технику'}</DrawerTitle>
            </DrawerHeader>
            <DrawerBody overflowY="auto">{formContent}</DrawerBody>
            <DrawerFooter gap={2} borderTopWidth="1px">
              <Button variant="subtle" colorPalette="gray" disabled={loading} type="button" onClick={() => onOpenChange(false)}>
                Закрыть
              </Button>
              <Button type="submit" disabled={loading} loading={loading}>
                {isEdit ? 'Сохранить' : 'Добавить'}
  ...
  return (
    <DialogRoot open={open} onOpenChange={(e) => onOpenChange(e.open)} size={{ base: 'xs', md: 'lg' }} placement="center">
      <DialogContent>
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogHeader>
