...
export function EquipmentList() {
  ...
  const [sortBy, setSortBy] = useState<
    EquipmentSortField | undefined
  >(undefined);
  const [sortOrder, setSortOrder] =
    useState<EquipmentSortOrder>('asc');
  const [selectedIds, setSelectedIds] = useState<
    Set<string>
  >(new Set());
  const [zoneDialogOpen, setZoneDialogOpen] =
    useState(false);
  ...
  const items = useMemo(() => {
    ...
    if (statusFilter) {
      list = list.filter(
        (i) => i.current_status === statusFilter,
      );
    }

    if (typeFilter) {
      list = list.filter(
        (i) => i.equipment_type === typeFilter,
      );
    }
    ...
    arr.sort((a, b) => {
      ...
      switch (sortBy) {
        case 'brand_model': {
          const sa =
            `${a.brand_name ?? ''} ${a.model ?? ''}`.trim();
          const sb =
            `${b.brand_name ?? ''} ${b.model ?? ''}`.trim();
          cmp = sa.localeCompare(sb);
          break;
        }
        case 'serial_number':
          cmp = (a.serial_number ?? '').localeCompare(
            b.serial_number ?? '',
          );
          break;
        case 'garage_number':
          cmp = (a.garage_number ?? '').localeCompare(
            b.garage_number ?? '',
          );
          break;
        case 'equipment_type':
          cmp = (a.equipment_type ?? '').localeCompare(
            b.equipment_type ?? '',
          );
          break;
        case 'zone':
        ...
        case 'commissioned_at': {
          const da = a.commissioned_at
            ? Date.parse(a.commissioned_at)
            : NaN;
          const db = b.commissioned_at
            ? Date.parse(b.commissioned_at)
            : NaN;
          const va = Number.isFinite(da)
            ? da
            : sortOrder === 'asc'
              ? 1e18
              : -1;
          const vb = Number.isFinite(db)
            ? db
            : sortOrder === 'asc'
              ? 1e18
              : -1;
        ...
        case 'current_status':
          cmp = (a.current_status ?? '').localeCompare(
            b.current_status ?? '',
          );
          break;
      }
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredCount / PER_PAGE),
  );
  ...
  const rangeStart =
    filteredCount === 0 ? 0 : (page - 1) * PER_PAGE + 1;
  ...
  const isAllSelected =
    pageItems.length > 0 &&
    pageItems.every((i) => selectedIds.has(i.id));
  const isSomeSelected = pageItems.some((i) =>
      ...
      <div className='mb-4 flex flex-col flex-wrap gap-4 md:flex-row'>
        ...
        <div className='flex flex-wrap items-center gap-2'>
          ...
          <Select
            ...
            onValueChange={(v) =>
              setStatusFilter(fromSelectAll(v))
            }
          >
      ...
      {isLoading && !data ? (
        ...
        <div className='flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-border py-12 text-center'>
          ...
          <p className='max-w-md text-sm text-muted-foreground'>
            Складская техника: бренды задаются в разделе
