...
const WAREHOUSE_SUGGESTIONS = [
  ...
  "Какие ряды в текущем layout?",
  "Краткая сводка по остаткам",
  "Какие задачи по обслуживанию техники на этой неделе?",
]
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-none border-0 bg-background md:rounded-xl md:border md:border-border md:shadow-sm">
          ...
          <div className="relative flex min-h-0 flex-1 flex-col divide-y divide-border overflow-hidden">
                  ...
                  <div className="flex min-h-0 flex-1 flex-col items-center justify-center px-4 py-8 motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-1 md:py-12">
                    ...
                    >
                      ...
                      <p className="mx-auto max-w-[32rem] text-center text-xs text-muted-foreground/90">
                        Ответ на сервере с учётом прав и данных склада.
                        Веб-поиск не используется.
                      </p>
                    </div>
                  </div>
                ) : (
            ...
            <div className="grid shrink-0 gap-4 bg-background pt-4">
              <Suggestions className="px-3 md:px-5">
                {WAREHOUSE_SUGGESTIONS.map((text) => (
                  <Suggestion
                    key={text}
                    suggestion={text}
                    disabled={historyLocked}
                    onClick={() => void send({ text, files: [] })}
                  />
                ))}
              </Suggestions>
