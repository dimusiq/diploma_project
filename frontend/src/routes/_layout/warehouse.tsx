...
type WarehouseSearch = z.infer<
  typeof warehouseSearchSchema
>;

function getItemsQueryOptions(params: WarehouseSearch) {
  const { page, search, category_id, sort_by, sort_order } =
    params;
  return {
    queryFn: () =>
...
function WarehouseTable() {
  ...
  const [selectedIds, setSelectedIds] = useState<
    Set<string>
  >(new Set());
  const [moveDialogOpen, setMoveDialogOpen] =
    useState(false);
  const [massEditDialogOpen, setMassEditDialogOpen] =
    useState(false);
  ...
  const handleSort = (field: SortField) => {
    (
      navigate as unknown as (opts: {
        search: (prev: WarehouseSearch) => WarehouseSearch;
      }) => void
    )({
      search: (prev) => ({
        ...prev,
  ...
  const setSearchParams = (
    updates: Partial<WarehouseSearch>,
  ) =>
    (
      navigate as unknown as (opts: {
        search: (prev: WarehouseSearch) => WarehouseSearch;
  ...
  const [optimisticItems, addOptimisticRemove] =
    useOptimisticItems(items);
  ...
  const isSomeSelected = optimisticItems.some((i) =>
