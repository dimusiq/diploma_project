...
Здесь хранится то, что должно переживать перезапуск процесса: план склада,
парк устройств, справочник товаров, документы (заказы, задания, движения) и
журнал событий. Высокочастотное состояние (координаты, заряд, показания
датчиков) живёт в памяти движка и в снимке для клиента — в БД не пишется.
